...
export default class Config {
  constructor(config = {}) {
    ...
    const rawCameras = new Map(
      (config.cameras || []).filter((camera) => camera?.name).map((camera) => [camera.name, structuredClone(camera)])
    );
    ...
    config.cameras = config.cameras
      .map((camera) => {
        ...
        const rawCamera = rawCameras.get(camera.name);
        const rawMotionTimeout = rawCamera?.motionTimeout ?? camera.motionTimeout;
        const rawMotionDelay = rawCamera?.motionDelay ?? camera.motionDelay;

        camera.motionTimeout =
          Number.isFinite(rawMotionTimeout) && rawMotionTimeout >= 0 && rawMotionTimeout <= 170 ? rawMotionTimeout : 15;

        camera.motionDelay =
          Number.isFinite(rawMotionDelay) && rawMotionDelay >= 0 && rawMotionDelay <= 10 ? rawMotionDelay : undefined;

        camera.prebufferLength =
          Number.isFinite(camera.prebufferLength) && camera.prebufferLength >= 4 && camera.prebufferLength <= 8
            ? camera.prebufferLength
            : 4;
