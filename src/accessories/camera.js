...
export default class CameraDelegate {
  constructor(api, accessory, config, cameraUi, handler) {
    ...
    this.controller = new this.api.hap.CameraController({
      cameraStreamCount: this.accessory.context.config.hsv
        ? 1
        : this.accessory.context.config.videoConfig.maxStreams || 2,
      delegate: this,
      streamingOptions: {
  ...
  async reservePorts(count = 1, type = 'udp', attemptNumber = 0) {
    ...
    const ports = [port];
    const tryAgain = () => this.reservePorts(count, type, attemptNumber + 1);
