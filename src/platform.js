...
const PLUGIN_NAME = 'homebridge-camera-vision';
const PLATFORM_NAME = 'CameraVision';
const PLATFORM_ALIASES = new Set([PLATFORM_NAME, 'CameraUI']);
const ENV_OPTIONS = {
  moduleName: 'homebridge-camera-vision',
  moduleVersion: packageJson.version,
  global: true,
  sudo: true,
};

export class HomebridgeCameraVision {
  ...
  async changeConfig(configJson) {
    ...
    try {
      ...
      for (const index in config.platforms) {
        if (PLATFORM_ALIASES.has(config.platforms[index].platform)) {
      ...
      const configPlatform = config.platforms.find((config_) => PLATFORM_ALIASES.has(config_.platform));
